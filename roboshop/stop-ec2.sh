...
stop_server(){
     ...
     #sed -e "s/component/$component-${env}/" -e "s/Ipaddress/${private_IP}/" route53del.json > /tmp/DNS1.json
     aws route53 change-resource-record-sets --hosted-zone-id ${Hosted_zone_id} --change-batch file:///tmp/DNS1.json
